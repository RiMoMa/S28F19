...
\vspace{-0.5cm}
\begin{itemize}
\only<1->{\item The current research is on automatic strategies to quantify nuclear pleomorphism in breast cancer(NPBca).
}
\only<2->{\item NPBca is an indicator of the aggressiveness of the disease
}
\only<3->{\item NP has a low inter-observer agreement ($0.3<\kappa<0.5$)}
\end{itemize}
