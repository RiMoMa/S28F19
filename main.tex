...
\date{\today}
\titlegraphic{\includegraphics[height=2.2cm,width=3.5cm]{graphics/cimalablogo.png}}
...
\vspace{-0.5cm}
\begin{itemize}
\only<1->{\item The current research is onto automatic strategies to quantify nuclear pleomorphism in breast cancer(NPBca).
}
\only<2->{\item NPBca is an indicator of the aggressiveness of the disease
}
\only<3->{\item NP has a low inter-observer agreement ($0.3<\kappa<0.5$)\blfootnote{Kappa coefficient ($\kappa$) measures the agreement between examiners $\kappa=\{0,1\}$ }}
\end{itemize}
...
\only<1->{\centering \includegraphics[width=0.5\textwidth]{imagenes/nucleipatches.png}}
\end{frame}
...
\only<2>{\vspace*{-4cm}\begin{block}{}
The BoF with the multiscale descriptor presents a better performance that the SPYA!, a new experiment of this without autoencoder is evaluated...
\end{block}}
\end{frame}
...
\only<2>{\vspace{-2cm}
\begin{block}{}
Then, we obtain a better model of NP using the spatial pyramidal representation aproach.
...
\only<2>{\vspace*{-5cm}
\begin{block}{}{
Two main diferences of the previous aproach are presented: 1) The clustering step, 2) A topic representation}
\end{block}}
